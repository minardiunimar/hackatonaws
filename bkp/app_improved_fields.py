#!/usr/bin/env python3
"""
Frontend Web Real para Processador de Documentos Pessoais - Versão com Campos Estruturados
Integração completa com document_processor_improved.py usando AWS Textract
Sempre exibe todos os campos estruturados detectados
"""
...
def sanitize_basic_fields(result):
    """Sanitiza apenas os campos básicos, mantendo campos estruturados intactos"""
    if not result or not isinstance(result, dict):
        return result
    
    sanitized = result.copy()
    
    # Sanitizar apenas campos básicos para demonstração
    basic_fields_to_sanitize = ['nome', 'cpf', 'rg']
    
    for field in basic_fields_to_sanitize:
        if field in sanitized and sanitized[field] and sanitized[field] != 'Não encontrado':
            sanitized[f'{field}_original'] = sanitized[field]  # Manter original para logs
            
            if field == 'nome':
                sanitized[field] = '<NOME_COMPLETO>'
            elif field == 'cpf':
                sanitized[field] = '<XXX.XXX.XXX-XX>'
            elif field == 'rg':
                sanitized[field] = '<XX.XXX.XXX-X>'
    
    # MANTER campos estruturados intactos para exibição completa
    # Não sanitizar campos_estruturados
...
def index():
    """Página principal"""
    return render_template('index_improved_fields.html')
...
def upload_file():
    ...
    try:
        ...
        # Sanitizar apenas campos básicos, manter campos estruturados
        display_result = sanitize_basic_fields(result)
        ...
        # Garantir que campos estruturados estejam sempre presentes
        if 'campos_estruturados' not in display_result:
            display_result['campos_estruturados'] = {}
        
        # Contar campos estruturados válidos (não vazios)
        valid_structured_fields = {k: v for k, v in display_result.get('campos_estruturados', {}).items() 
                                 if v and str(v).strip() != ''}
        
        display_result['campos_estruturados_count'] = len(valid_structured_fields)
        display_result['has_structured_data'] = len(valid_structured_fields) > 0
        
        # Log dos campos estruturados para debug
        logger.info(f"Campos estruturados detectados ({len(valid_structured_fields)}): {list(valid_structured_fields.keys())}")
...
def health_check():
    ...
    return jsonify({
        ...
        'processor_version': 'improved_with_structured_fields',
        'aws_textract_status': aws_status,
        'feature': 'structured_fields_display'
    })
    ...
    try:
        ...
        return jsonify({
            ...
            'features': [
                ...
                'Complete structured fields display'
            ]
        })
    except Exception as e:
        return jsonify({
